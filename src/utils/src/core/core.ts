...
): [Record<string, T>, string]
{
  ...
  if (Array.isArray(active[lastKey])) active[lastKey].push(val)
  else active[lastKey] = val
...
type StepResult<T> = T | Error
type StepHandler<Prev, Out> = (input: Prev, done: (result: Out | Error) => void) => void | Promise<void>

type StepOutputs<T extends readonly StepHandler<any, any>[]> = {
...
/**
 *
 * 链式执行多个步骤函数，每个步骤函数接收上一步结果和 done 回调，通过调用 done 传递结果或错误
 * @author      Yuluo
 * @link        https://github.com/YuluoY
 * @date        2025-06-08
 * @template T 步骤函数数组类型
 * @param   {T}                 steps     步骤函数数组
 * @param   {ChainOptions}      [opts]    执行选项
 ...
 * ], { timeout: 1000 })
 * console.log(res) // [Error: chain: step 0 timeout, undefined]
 */
  ...
  steps.reduce<Promise<void>>(async(prevPromise, step, index) =>
  {
    ...
    return new Promise<void>(async resolve =>
    {
      ...
      const done = (result?: unknown) =>
      {
        ...
        if (result instanceof Error) hasError = true
